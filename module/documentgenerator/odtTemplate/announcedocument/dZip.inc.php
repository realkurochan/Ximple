...
class dZip{
	...
	Function addDir($dirname, $cdirname, $fileComments=''){
		if(substr($dirname, -1) != '/')
			$dirname .= '/';
		if(substr($cdirname, -1) != '/')
			$cdirname .= '/';
		if ($dirname != "./")
			$this->addFile(false, str_replace('./', '', "$cdirname"), $fileComments);
			...
			while (false !== ($file = readdir($handle))) { 
				if ($file != "." && $file != "..")
				{
					if (is_dir("$dirname$file"))
						$this->addDir("$dirname$file","$cdirname$file" , $fileComments);
					else
						$this->addFile(str_replace('./', '', "$dirname$file"), str_replace('./', '', "$cdirname$file"));
				}
			}
			closedir($handle);
		}
